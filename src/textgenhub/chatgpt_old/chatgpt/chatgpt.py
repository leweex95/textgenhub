"""
ChatGPT provider (legacy puppeteer-based via chatgpt-attach)
Simple wrapper over Node.js CLI
"""
...
def ask(
    ...
    """
    Send a prompt to ChatGPT and get a response using legacy puppeteer method.
    ...
    """
    provider = SimpleProvider("chatgpt_old", "chatgpt_old_cli.cjs")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="ChatGPT CLI (legacy puppeteer) via Node.js wrapper")
    parser.add_argument("--prompt", type=str, required=True, help="Prompt to send to ChatGPT")
    parser.add_argument(
        "--headless",
        type=lambda x: x.lower() == "true",
        default=True,
        help="Run Node in headless mode",
    )
    parser.add_argument(
        "--remove-cache",
        type=lambda x: x.lower() == "false",
        default=False,
        help="Remove cache on cleanup",
    )
